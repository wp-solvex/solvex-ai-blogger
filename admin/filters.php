...
class Filters {
	...
	public function add_campaign_filter(): void {
		...
		$campaigns         = solvex_aib_get_all_campaigns();
		$selected_campaign = isset( $_GET['solvex_aib_campaign_id'] ) ? absint( $_GET['solvex_aib_campaign_id'] ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Not required as handled by absint().

		if ( ! empty( $campaigns ) ) {
			echo '<select name="solvex_aib_campaign_id">';
	...
	public function filter_posts_by_campaign( $query ) {
		...
		if ( ! isset( $_GET['solvex_aib_campaign_id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Not required as handled by absint().
			return $query;
		}

		$campaign_id = absint( $_GET['solvex_aib_campaign_id'] ?? 0 ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Not required as handled by absint().
	...
	public function add_campaign_column( $columns ) {
		...
		$columns['solvex_aib_campaign'] = __( 'Campaign', 'solvex-ai-blogger' );
		return $columns;
	}
	...
	public function show_campaign_column_content( $column, $post_id ): void {
		if ( $column === 'solvex_aib_campaign' ) {
			$campaign_id = absint( get_post_meta( $post_id, 'solvex_aib_campaign_id', true ) ?? 0 );
			...
			if ( $campaign_id ) {
				...
				if ( $campaign_title ) {
					...
					$filter_url = add_query_arg( 'solvex_aib_campaign_id', $campaign_id, $filter_url );
