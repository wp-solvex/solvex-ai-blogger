...
const createPostDraft = async ( title, response, options = {} ) => {
	...
	formData.append( 'security', options.nonce || wpsolvex_autoaiblogger_localized_data?.admin_nonce || '' );
	...
	try {
		const result = await apiFetch( {
			url: options.ajaxUrl || wpsolvex_autoaiblogger_localized_data?.ajax_url || '',
			method: 'POST',
			body: formData,
		} );
