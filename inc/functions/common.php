...
function solvex_aib_clean_data( $data ) {
	if ( is_array( $data ) ) {
		return array_map( 'solvex_aib_clean_data', $data );
	}
	...
	try {
		...
		if ( ! is_wp_error( $campaigns ) && ! empty( $campaigns ) ) {
			foreach ( $campaigns as $campaign ) {
				...
				if ( ! $campaign instanceof WP_Post || $campaign->post_type !== SOLVEX_AIB_CPT_CAMPAIGN ) {
					continue;
				}
...
function solvex_aib_get_array_depth( array $array ): int {
	...
	foreach ( $array as $value ) {
		if ( is_array( $value ) ) {
			$depth = solvex_aib_get_array_depth( $value ) + 1;
...
function solvex_aib_get_post_types() {
	...
	try {
		$queried_post_types = array_keys(
			get_post_types(
				apply_filters(
					'solvex_aib_post_types_query_args',
					[
						'public'   => true,
						'_builtin' => false,
					]
		...
		$excluded_post_types = apply_filters(
			'solvex_aib_excluded_post_types',
			[
				SOLVEX_AIB_CPT_CAMPAIGN,
				'sfwd-assignment',
				'sfwd-essays',
				'sfwd-transactions',
				'sfwd-certificates',
...
function solvex_aib_get_post_creation_api_response( $keywords, $max_content_words, $site_persona_details, $campaign_id = 0, $campaign_name = '', $image_count = 1 ) {
	...
	try {
		...
		$api_url   = SOLVEX_AIB_CAMPAIGN_POST_API;
		...
		$args     = [
			...
			'headers'     => [
				...
				'User-Agent'   => 'Solvex-AI-Blogger/' . SOLVEX_AIB_VERSION,
			],
		...
		if ( is_array( $data ) ) {
			$data = solvex_aib_sanitize_api_response( $data );
		}
...
function solvex_aib_sanitize_api_response( $data ) {
	...
	foreach ( $data as $key => $value ) {
		...
		if ( is_array( $value ) ) {
			$sanitized[ $clean_key ] = solvex_aib_sanitize_api_response( $value );
...
function solvex_aib_track_post_view( $post_id ): void {
	...
	$is_campaign_post = get_post_meta( $post_id, 'solvex_aib_campaign_id', true );
	if ( ! $is_campaign_post ) {
		return;
	}
...
function solvex_aib_log_campaign_error( $campaign_id, $error_type, $error_message, $context = [] ): void {
	...
	$timestamp_data = solvex_aib_create_timestamp_data();
...
function solvex_aib_get_campaign_error_logs( $campaign_id, $limit = 20 ): array {
	...
	foreach ( $error_logs as $index => $log ) {
		...
		$user_friendly_message = solvex_aib_get_user_friendly_error_message( $error_type, $log['message'] ?? '' );
		$error_solution        = solvex_aib_get_error_solution_suggestion( $error_type );
